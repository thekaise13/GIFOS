    ...
    fetch(`https://api.giphy.com/v1/gifs/search/tags?api_key=vPpkELaH3rnKb94KI9Mz8KU8apj5qZjr&q=${userData}`)
        ...
        .then((gifs) => {
            ...
            if (userData) {
                ...
                let lupas = document.querySelectorAll('.fa-search')
                for (let i = 0; i < lupas.length; i++) {
                    lupas[i].addEventListener('click', (nodo) => {
                        let DataLi = nodo.path[1].outerText
                        if (DataLi === "") {
                            DataLi = inputBox.value
                            console.log('entro');
                        }
...
let search = document.querySelectorAll('.fa-search');

for (let i = 0; i < search.length; i++) {
    search[i].addEventListener('click', (nodo) => {
